...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        img_lq = cv2.imread(f'{args.folder_lq}/{imgname}X{args.scale}{imgext}', read_flag).astype(
            np.float32) / 255.
    ...
    if read_flag == cv2.IMREAD_GRAYSCALE:
        img_lq = np.expand_dims(img_lq, axis=2)  # HxWx1
        img_gt = np.expand_dims(img_gt, axis=2)  # HxWx1
