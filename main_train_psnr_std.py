...
# 设置 API key
os.environ['WANDB_API_KEY'] = "49fd2d67dea4a38a655d8611f52e9725c9a11975"
...
def main():
    ...
    parser.add_argument('--opt', type=str, default="options/swinir/train_swinir_sr_classical_std_gan.json", help='Path to option JSON file.')
    ...
    opt = option.parse(parser.parse_args().opt, args, is_train=True)
    ...
    args.folder_lq = opt['datasets']['test']['dataroot_L']
    args.folder_gt = opt['datasets']['test']['dataroot_H']
    args.model_save_dir = f"./superresolution/{opt['task']}/models/improved/"
    ...
    if opt['rank'] == 0 and not args.test:
        wandb.init(project="SwinIR", entity="barryzp")
