...
class ModelGAN(ModelBase):
    ...
    def save_better_model(self, psnr, args, current_step):
        ...
        model_tag = f"epc_{current_step}_psnr_{psnr}"

        self.save_network(save_dir, self.netG, 'G', model_tag)
        self.save_network(save_dir, self.netD, 'D', model_tag)
        if self.opt_train['E_decay'] > 0:
            self.save_network(save_dir, self.netE, 'E', model_tag)
        if self.opt_train['G_optimizer_reuse']:
            self.save_optimizer(save_dir, self.G_optimizer, 'optimizerG', model_tag)
        if self.opt_train['D_optimizer_reuse']:
            self.save_optimizer(save_dir, self.D_optimizer, 'optimizerD', model_tag)
